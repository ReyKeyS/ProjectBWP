    ...
    <form action="" method="POST">
        ...
        <div class="pt-20 w-11/12 mx-auto flex flex-col">
            <div class="flex justify-center">
                ...
                <div class="text-3xl sm:text-4xl font-bold bg-gradient-to-r from-purple-700 to-blue-600 bg-clip-text text-transparent">TRANSACTION DETAIL</div>
            </div>
            <div class="w-full sm:w-1/2 p-5 mx-auto my-4 flex flex-col border rounded-xl shadow-xl">
                <div class="text-sm sm:text-xl flex w-full"><a class="text-slate-400"><?=$dataHTRANS["invoice"]?></a><a class="ml-auto"><?=substr($dataHTRANS["tanggal"], 0, 10)?> | <?=substr($dataHTRANS["tanggal"], 11)?></a></div>
                <div class="text-2xl font-semibold">Product Details</div>
                <?php
                    $totalSubtotal = 0;
                    while($row = mysqli_fetch_row($queryDTRANS)){
                ?>  
                <div class="flex border rounded-xl my-2 sm:mx-10 shadow-sm">
                    <div class="w-1/4">
                        <img src="<?=$row[0]?>" alt="" class="w-24 h-24 mx-auto">
                    </div>
                    <div class="w-2/4 flex flex-col">
                        <div class="text-xl sm:text-3xl font-semibold"><?=$row[1]?></div>
                        <div class="text-xs sm:text-base text-slate-600"><?=$row[2]?> x Rp <?=number_format($row[3])?></div>
                    </div>
                    <div class="w-1/4 flex flex-col">
                        <div class="text-sm sm:text-lg">Subtotal</div>
                        <div class="text-lg sm:text-2xl font-bold">Rp <?=number_format($row[2]*$row[3])?></div>
                    </div>
                </div>
                <?php
                ...
                ?>
                <div class="text-sm sm:text-lg">Total Price : Rp <?=number_format($totalSubtotal)?></div>
                <div class="text-sm sm:text-lg">Build Service : Rp <?php if ($dataHTRANS["dirakit"] == 0) echo "0"; else echo "120,000"; ?></div>
                <div class="font-bold text-lg sm:text-2xl">Grand Total : Rp <?=number_format($dataHTRANS["total"])?></div>
                <div class="flex ml-auto">
                    <?php
                        if($dataHTRANS["status"]=="1"){
                    ?>
                        <button type="button" data-modal-toggle="defaultModal" class="px-2 py-1 w-16 sm:px-3 sm:py-2 sm:w-20 rounded-xl mr-3 text-white font-semibold bg-red-500 hover:bg-red-600">Cancel</button>
                    <?php
                        }
                    ?>
                    <a href="history.php" class="text-sm sm:text-base font-bold mb-2 my-auto bg-gradient-to-r from-purple-700 to-blue-600 bg-clip-text text-transparent hover:bg-gradient-to-r hover:from-purple-900 hover:to-blue-800 hover:bg-clip-text hover:text-transparent hover:cursor-pointer">Back to History</a>
                </div>
            </div>
        </div>
